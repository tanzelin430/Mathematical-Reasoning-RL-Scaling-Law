...
Plots evaluation performance for 14B base model with each eval as a separate line

Usage:
  python run_plot_multi-subplot_oodid.py --eval-group in_domain --x-columns C --metrics ErrRate --data-source base
  python run_plot_multi-subplot_oodid.py --eval-group out_of_domain --x-columns C N --metrics ErrRate Score --data-source instruct
...
with one line per evaluation for the 14B base model.
"""
...
def main():
    ...
    N = 14e9
    # Filter for 14B model only
    df = df[df['N'] == N].copy()
    if len(df) == 0:
        print("No data found for 14B model!")
        return
    ...
    for x_column in x_columns:
        ...
        for metric in metrics:
            ...
            filename = config.OUTPUT_BASE_DIR / f"14B_{data_source}_{eval_group}_{x_column}_{metric_label_clean}.pdf"
