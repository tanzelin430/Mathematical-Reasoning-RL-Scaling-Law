...
def main():
    ...
    def logistic_e0(N, A_e0, r_e0, N0_e0, B_e0):
        """Logistic函数用于E0(N)：E0(N) = A_e0 / (1 + exp(r_e0 * (N - N0_e0))) + B_e0"""
        return A_e0 / (1 + np.exp(r_e0 * (N - N0_e0))) + B_e0

    def global_model(params, N, log10_E):
        """双Logistic拟合函数：log10(ErrRate) = -k(N) * log10(E) + E0(N)
        其中 k(N) = L / (1 + exp(-r * (N - N0)))
        E0(N) = A_e0 / (1 + exp(r_e0 * (N - N0_e0))) + B_e0
        """
        # k(N)的参数：L, r, N0
        L, r, N0 = params[:3]
        # E0(N)的参数：A_e0, r_e0, N0_e0, B_e0
        A_e0, r_e0, N0_e0, B_e0 = params[3:7]

        k = logistic_k(N, L, r, N0)
        E0 = logistic_e0(N, A_e0, r_e0, N0_e0, B_e0)
    ...
    # E0(N) Logistic函数的参数
    A_e0_init = 0.06   # E0的振幅 (从初步观察：约0.05到-0.01的变化)
    r_e0_init = 1e-9   # E0的增长率
    N0_e0_init = 3e9   # E0的拐点位置 
    B_e0_init = -0.01  # E0的下渐近线
    ...
    if True:
        ...
        def fit_func(data, L, r, N0, A_e0, r_e0, N0_e0, B_e0):
            N, log10_E = data
            params = [L, r, N0, A_e0, r_e0, N0_e0, B_e0]
        ...
        # 构建初始参数和边界
        p0 = [L_init, r_init, N0_init, A_e0_init, r_e0_init, N0_e0_init, B_e0_init]
        lower_bounds = [0, 0, 0, 0, 0, 0, -1]  # k(N): L>0, r>0, N0>0; E0(N): A_e0>0, r_e0>0, N0_e0>0, B_e0>-1
        upper_bounds = [1, 1e-6, 1e12, 1, 1e-6, 1e12, 1]  # 合理的上界
        
        print(f"参数数量: {len(p0)} (3个k(N) Logistic参数 + 4个E0(N) Logistic参数)")
        ...
        # 提取拟合参数
        L_fit, r_fit, N0_fit, A_e0_fit, r_e0_fit, N0_e0_fit, B_e0_fit = popt
        ...
        print(f"\n✅ 双Logistic拟合成功！")
        print(f"\nk(N) Logistic函数参数:")
        print(f"  L = {L_fit:.6f} (上渐近线)")
        print(f"  r = {r_fit:.2e} (增长率)")
        print(f"  N0 = {N0_fit:.2e} (拐点位置)")
        print(f"  k(N) = {L_fit:.6f} / (1 + exp(-{r_fit:.2e} * (N - {N0_fit:.2e})))")
        
        print(f"\nE0(N) Logistic函数参数:")
        print(f"  A_e0 = {A_e0_fit:.6f} (振幅)")
        print(f"  r_e0 = {r_e0_fit:.2e} (增长率)")
        print(f"  N0_e0 = {N0_e0_fit:.2e} (拐点位置)")
        print(f"  B_e0 = {B_e0_fit:.6f} (下渐近线)")
        ...
        # 计算并显示每个模型大小对应的E0值
        print(f"\n每个模型大小对应的E0(N)预测值:")
        E0_fit_list = [logistic_e0(n_val, A_e0_fit, r_e0_fit, N0_e0_fit, B_e0_fit) for n_val in unique_N_list]
        ...
        for ms in unique_model_sizes:
            ...
            # 计算E0值（使用Logistic函数）
            E0_local = logistic_e0(N_val, A_e0_fit, r_e0_fit, N0_e0_fit, B_e0_fit)
    ...
    title_text = (r"Dual-Logistic Model: $\log_{10}ErrRate = -k(N) \cdot \log_{10}E + E_0(N)$" + "\n" +
                 r"$k(N) = \frac{L}{1 + \exp(-r \cdot (N - N_0))}$" + "\n" +
                 r"$E_0(N) = \frac{A}{1 + \exp(r_{e0} \cdot (N - N_{0e0}))} + B$")
