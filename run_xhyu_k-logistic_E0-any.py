...
def main():
    ...
    # ---------------------------
    # 模型 0 (Logistic拟合版)：log10(ErrRate) = -k(N) * log10(E) + E0(N)
    # 其中 k(N) = L / (1 + exp(-r * (N - N0)))，L, r, N0 为全局参数，每个N有独立的E0(N)
    # ---------------------------
    print("\n=== 模型0 Logistic拟合版：log10(ErrRate) = -k(N) * log10(E) + E0(N) ===")
    print("其中 k(N) = L / (1 + exp(-r * (N - N0)))")
    print("每个模型大小N都有独立的E0(N)参数")
    ...
    def logistic_k(N, L, r, N0):
        """Logistic函数：k(N) = L / (1 + exp(-r * (N - N0)))"""
        return L / (1 + np.exp(-r * (N - N0)))

    def global_model(params, N, log10_E, unique_N_list):
        """全局拟合函数：log10(ErrRate) = -k(N) * log10(E) + E0(N)
        每个N都有独立的E0参数
        """
        # 前3个参数是Logistic函数的参数
        L, r, N0 = params[:3]
        ...
        k = logistic_k(N, L, r, N0)
        
        # 为每个N分配对应的E0
        E0 = np.zeros_like(N)
        for i, n_val in enumerate(unique_N_list):
            mask = np.abs(N - n_val) < 1e-6  # 数值比较的容差
            E0[mask] = E0_values[i]
    ...
    # 初始参数估计
    L_init = 0.06      # 最大k值（稍微超过当前观测的最大值）
    r_init = 2e-10     # 增长率
    N0_init = 5e9      # 拐点位置（中等模型大小）
    # 为每个N初始化一个E0值
    E0_init_list = [0.4] * n_models  # 每个模型都从0.4开始
    ...
    if True:
        ...
        def fit_func(data, L, r, N0, *E0_values):
            N, log10_E = data
            params = [L, r, N0] + list(E0_values)
            return global_model(params, N, log10_E, unique_N_list)
        ...
        # 构建初始参数和边界
        p0 = [L_init, r_init, N0_init] + E0_init_list
        lower_bounds = [0, 0, 0] + [-np.inf] * n_models
        upper_bounds = [1, 1e-6, 1e12] + [np.inf] * n_models
        
        print(f"参数数量: {len(p0)} (包括3个Logistic参数 + {n_models}个E0参数)")
        ...
        L_fit, r_fit, N0_fit = popt[:3]
        E0_fit_list = popt[3:]
        
        # 计算全局R²
        y_pred_all = global_model(popt, N_all_data, log10_E_all, unique_N_list)
        ...
        print(f"\n✅ Logistic拟合成功！")
        print(f"拟合参数:")
        print(f"  L = {L_fit:.6f} (Logistic函数的上渐近线)")
        print(f"  r = {r_fit:.2e} (增长率参数)")
        print(f"  N0 = {N0_fit:.2e} (拐点位置)")
        print(f"全局 R² = {r2_global:.4f}")
        print(f"\nLogistic函数形式: k(N) = {L_fit:.6f} / (1 + exp(-{r_fit:.2e} * (N - {N0_fit:.2e})))")
        
        print(f"\n每个模型大小的独立E0参数:")
        for i, n_val in enumerate(unique_N_list):
            print(f"  N = {n_val:.2e}: E0 = {E0_fit_list[i]:.6f}")
        ...
        try:
            ...
            popt_e0_logistic, pcov_e0_logistic = curve_fit(
                ...
                bounds=([0, 0, 0, -np.inf], [1, 1e-6, 1e12, np.inf]),
                maxfev=10000
            )
            ...
            plt.subplot(2, 2, 1)
            plt.scatter(unique_N_list, E0_fit_list, color='red', s=80, alpha=0.8, 
                       marker='o', edgecolor='black', linewidth=1, zorder=5, label='实际E0值')
            ...
            if 'r2_e0_linear' in locals():
                ...
                plt.plot(N_smooth, E0_smooth_linear, 'g--', linewidth=2, label=f'线性 (R²={r2_e0_linear:.3f})')
            
            # 对数拟合线
            if 'r2_e0_log' in locals():
                E0_smooth_log = a_e0_log * np.log(N_smooth) + b_e0_log
                plt.plot(N_smooth, E0_smooth_log, 'm:', linewidth=2, label=f'对数 (R²={r2_e0_log:.3f})')
            
            plt.xscale('log')
            plt.xlabel('Model Size N (parameters)')
            plt.ylabel('E0 parameter')
            plt.title('E0(N) 拟合比较 - 对数尺度')
            plt.legend()
            plt.grid(True, alpha=0.3)
            ...
            plt.subplot(2, 2, 2)
            plt.scatter([n/1e9 for n in unique_N_list], E0_fit_list, color='blue', s=80, alpha=0.8,
                       marker='s', edgecolor='black', linewidth=1, zorder=5, label='实际E0值')
            ...
            plt.xscale('log')
            plt.xlabel('Model Size N (parameters)')
            plt.ylabel('Logistic 拟合残差')
            plt.title('Logistic拟合残差分析')
            plt.grid(True, alpha=0.3)
        ...
        for ms in unique_model_sizes:
            ...
            N_val = float(subdf['N'].iloc[0])
            k_logistic = logistic_k(N_val, L_fit, r_fit, N0_fit)  # 使用Logistic拟合的k(N)关系
            
            # 找到对应的E0值
            n_index = unique_N_list.index(N_val)
            E0_local = E0_fit_list[n_index]
            
            # 计算该模型大小的R²
            y_pred = global_model(popt, np.array([N_val] * len(x)), x, unique_N_list)
            ...
            print(f"N={N_val:.3g}: k={k_logistic:.6f} (Logistic), E0={E0_local:.6f} (独立), R2(log10 ErrRate)={r2_local:.4f}")
            
            # 画拟合线
            E_grid = np.logspace(np.log10(E_vals.min()), np.log10(E_vals.max()), 200)
            x_grid = np.log10(E_grid)
            y_grid = global_model(popt, np.array([N_val] * len(x_grid)), x_grid, unique_N_list)
    ...
    plt.title(r"Model0 Logistic: $\log_{10}ErrRate = -k(N) \log_{10}E + E_0$")
    plt.legend(title="model size", loc="best")
    plt.tight_layout()
    ...
    print(f"\n=== ErrRate 模型拟合完成 ===")
    print("模型：Logistic拟合 log10(ErrRate) = -k(N) * log10(E) + E0(N)")
    print(f"其中 k(N) = {L_fit:.6f} / (1 + exp(-{r_fit:.2e} * (N - {N0_fit:.2e})))")
    print("每个N都有独立的E0参数")
    if model0_stats:
