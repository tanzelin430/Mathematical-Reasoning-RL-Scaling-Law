...
class SimpleLinearLookupFit:
    ...
    def get_params_array(self) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        """Get all parameters as arrays for analysis."""
        N_values = sorted(set(self.k_lookup.keys()) & set(self.E0_lookup.keys()))
        N_arr = np.array(N_values)
        k_arr = np.array([self.k_lookup[N] for N in N_values])
        E0_arr = np.array([self.E0_lookup[N] for N in N_values])
...
def fit_simple_lookup(df: pd.DataFrame, eval_name: str = "holdout_score") -> SimpleLinearLookupFit:
    """
    Convenience function to fit the simple lookup model.
    """
    fitter = SimpleLinearLookupFit()
    fitter.fit(df, eval_name)
    return fitter
