...
STEM_JUDGE_MODEL_PATH="/home/myid/yz44466/Agentic-RL-Scaling-Law/general-verifier"  # Path to STEM Judge model
STEM_JUDGE_GPU=0  # GPU for STEM LLM Judge (separate from training GPUs)

# GPU Configuration
TRAINING_GPUS="1,2,3,4,5,6,7,8"  # GPUs for training (excluding GPU 0 which is used by vLLM)
...
# Hardware Platform
num_nodes=1
n_gpus_per_node=8  # Using 7 GPUs for training (GPU 0 reserved for vLLM)
...
start_stem_judge() {
    echo "Starting STEM LLM Judge server for ${DOMAIN_NAME} training..."
...
# 🎯 Conditional startup: Only start if DOMAIN_NAME contains "stem"
if [[ "$DOMAIN_NAME" == *"stem"* ]]; then
    echo "STEM domain detected in training. Starting STEM LLM Judge..."
    start_stem_judge
else
    echo "No STEM domain in training (${DOMAIN_NAME}). Skipping STEM LLM Judge startup."
fi
