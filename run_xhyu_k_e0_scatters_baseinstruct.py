...
def main():
    ...
    # Configuration
    fitting_type = "E"  # "C" for L(N,C) fitting, "C_raw" for L(N,C_raw) fitting, "D" for L(N,D) fitting
...
def create_comparison_plots(all_results, fitting_type, display_names):
    ...
    # Create 2 subplots: k(N) and E0(N)
    ...
    for model_type, data in plot_data.items():
        ax1.scatter(data['N_values'], data['k_values'], 
                   ...
                   s=100, alpha=0.7, label=labels[model_type])
    
    ax1.set_xscale('log')
    ax1.set_xlabel('Model Size N')
    ax1.set_ylabel(f'k(N)')
    ax1.set_title(f'k(N) Comparison: L(N,{display_names[fitting_type]}) Fitting')
    ax1.legend()
    # ax1.grid(True, alpha=0.3)
    
    # Plot E0(N) comparison
    for model_type, data in plot_data.items():
        ax2.scatter(data['N_values'], data['E0_values'], 
                   color=colors[model_type], marker=markers[model_type], 
                   s=100, alpha=0.7, label=labels[model_type])
    
    ax2.set_xscale('log')
    ax2.set_xlabel('Model Size N')
    ax2.set_ylabel(f'E0(N)')
    ax2.set_title(f'E0(N) Comparison: L(N,{display_names[fitting_type]}) Fitting')
    ax2.legend()
    ...
    # Save the plot
    comparison_path = config.OUTPUT_BASE_DIR / f"base_vs_instruct_{fitting_type}_fitting_comparison.pdf"
